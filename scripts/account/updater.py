...
class RefreshPlanStats:
    ...
    def __init__(self):
        ...
        # 用户 activity_level 分布
        self.activity_distribution = [0] * 10
    ...
    def add_batch(self, rows: list[tuple]) -> list:
        ...
        rows 中每条记录格式为:
            (account_id, is_enabled, activity_level, next_refresh_at_unix, updated_at_unix)
        """
        due_ids = []

        for account_id, is_enabled, activity_level, next_refresh_at, updated_at in rows:
            ...
            if remaining_seconds == -1:
                ...
                self.total_count += 1
                self.buckets[0].append(account_id)
                continue
            ...
            if remaining_seconds < 86400:
                ...
                if hour_index < 24:
                    self.total_count += 1
                    self.buckets[hour_index].append(account_id)

        self.total_due += len(due_ids)
        return due_ids

    def add_locked_ids(self, locked_ids: list):
        """将成功获取锁的 ID 加入最终更新列表，并累加锁定计数"""
        self.update_list.extend(locked_ids)
        self.locked += len(locked_ids)
