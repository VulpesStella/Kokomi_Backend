...
"""
================================================================================
用户PVP数据缓存更新系统 (User Cache Service)
================================================================================
...
from updater import update_user_cache
from db_ops import (
    get_update_ids,
    read_ship_data,
    read_ship_record
...
async def main():
    ...
    通过调用外部API获取最新数据，将处理后的数据存储到MySQL数据库和Redis缓存中
    ...
    while True:
        ...
        try:
            ...
            if len_update_ids > 0:
                # 加载符合排行榜统计船只的数据
                ship_data = read_ship_data(mysql_connection)
                if isinstance(ship_data, str):
                    logger.error(f"Read ship data failed: {ship_data}")
                    raise
                # 读取船只相关字段的最高记录信息
                ship_record = read_ship_record(mysql_connection)
                if isinstance(ship_record, str):
